/*
 * MPEG-2 I+P frame decoder with Motion Compensation (CPU-only).
 *
 * Decodes the first I-frame as reference, then the first P-frame using
 * motion compensation + IDCT residual. Outputs each frame as PPM.
 *
 * Build:
...
// Decode one picture (I or P) into dst frame.
// For P-frames, ref must point to the forward reference frame.
static uint32
	...
	while (mpeg2_bits_available(bs, 32)) {
		...
		while (mbx < dec->mb_width && mpeg2_bits_available(bs, 8)) {
			...
			if (mb.address_increment > 1) {
				...
				if (is_p && ref) {
					// Skipped MBs in P-frame: copy from reference
			...
			if (mb.mb_type & MB_INTRA) {
				...
				// Reset MV prediction on intra MB in P-frame
				if (is_p) {
					dec->mv_pred_h = 0;
					dec->mv_pred_v = 0;
				}
				...
				// Inter MB with motion compensation
				...
				for (int c = 0; c < 2; c++) {
					...
					const uint8* ref_plane = (c == 0) ? ref->cb : ref->cr;
					uint32 cx = mbx * 8, cy = mby * 8;
					uint8 mc_ref_block[64];
...
int main(int argc, char** argv)
{
	...
	while (mpeg2_bits_available(&bs, 32)) {
		...
		if (code == MPEG2_SEQUENCE_HEADER_CODE) {
			...
			// For P-frame, start from reference (skipped MBs copy from ref)
			...
			// Update reference frame (I and P frames become references)
