/*
 * gpu_idct_bench — Standalone GPU GEMV sulle EU (no reboot needed).
 *
 * Opens the intel_extreme device, maps shared_info to get ring buffer
 * access, then calls media_pipeline_run_prefill_test() which does the
 * full GEMV sulle EU internally (400 blocks GPU vs CPU).
 *
...
// media_pipeline_run_prefill_test is declared in media_pipeline.h
// (it currently does the GEMV sulle EU despite the name)


int
main(int argc, char** argv)
{
	printf("=== GPU GEMV su GPU (Livello 2) (userspace, no reboot) ===\n\n");
	...
	printf("\nRunning GEMV sulle EU...\n\n");
	status_t st = media_pipeline_run_prefill_test();
