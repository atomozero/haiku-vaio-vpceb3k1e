...
intel_init_accelerant(int device)
{
	...
	// === X-TILING DIAGNOSTIC TEST ===
	// Test fence register read/write and aligned allocation.
	// NO display changes - just diagnostics in syslog.
	if (info.device_type.InGroup(INTEL_GROUP_ILK)) {
		uint32 fenceBase = INTEL_FENCE_BASE_965;
		uint32 fenceReg = fenceBase + 0 * INTEL_FENCE_SIZE;  // fence 0

		// Step 1: Read current fence 0 value
		uint32 origLo = read32(fenceReg);
		uint32 origHi = read32(fenceReg + 4);
		ERROR("TILING TEST: fence 0 @0x%x current: lo=0x%08" B_PRIx32
			" hi=0x%08" B_PRIx32 "\n", fenceReg, origLo, origHi);

		// Step 2: Write a test value and read back
		write32(fenceReg, 0);
		read32(fenceReg);
		write32(fenceReg, 0xDEAD0001);
		uint32 readBack = read32(fenceReg);
		ERROR("TILING TEST: wrote 0xDEAD0001, read back 0x%08" B_PRIx32
			" (%s)\n", readBack,
			readBack == 0xDEAD0001 ? "MATCH" : "MISMATCH");

		// Restore original value
		write32(fenceReg, origLo);
		write32(fenceReg + 4, origHi);
		read32(fenceReg);

		// Step 3: Test aligned allocation
		addr_t testBase;
		uint32 testSize = 8 * 1024 * 1024;  // 8MB
		status_t allocStatus = intel_allocate_memory(testSize, testSize,
			0, testBase);
		if (allocStatus == B_OK) {
			uint32 testOffset = testBase
				- (addr_t)info.graphics_memory;
			ERROR("TILING TEST: 8MB alloc OK, base=%p offset=0x%"
				B_PRIx32 " aligned=%s\n",
				(void*)testBase, testOffset,
				(testOffset & (testSize - 1)) == 0 ? "YES" : "NO");
			intel_free_memory(testBase);
		} else {
			ERROR("TILING TEST: 8MB aligned alloc FAILED (%s)\n",
				strerror(allocStatus));

			// Try without alignment
			allocStatus = intel_allocate_memory(testSize, 0, testBase);
			if (allocStatus == B_OK) {
				uint32 testOffset = testBase
					- (addr_t)info.graphics_memory;
				ERROR("TILING TEST: 8MB unaligned alloc OK, offset=0x%"
					B_PRIx32 "\n", testOffset);
				intel_free_memory(testBase);
			} else {
				ERROR("TILING TEST: 8MB unaligned alloc also FAILED\n");
			}
		}

		// Step 4: Report current framebuffer
		ERROR("TILING TEST: current FB offset=0x%" B_PRIx32
			" stride=%" B_PRIu32 "\n",
			info.frame_buffer_offset, info.bytes_per_row);
	}
	// === END TILING DIAGNOSTIC ===
